...
function* postStatusNf(body) {
  ...
  try {
    ...
    if (res.responseCode !== 200) {
      console.log("res !== 200 ... hyyyyyy====== ", res);
      if (res?.message) Toast.show(res.message, Toast.LONG);
      ...
      if (body?.data?.postHasVideo) {
        if (body?.data?.postHasVideo) {
          ...
          setTimeout(() => {
            Toast.show("Your video is uploaded successfully.", Toast.SHORT);
          }, processingDelay);
        } else {
...
function* fetchSinglePost({ data }) {
  ...
  try {
    ...
    if (res.responseCode !== 200) {
      console.log("res !=-0-0-0-0-0-0-0= 200 ... ", res);
...
function* editPost(params) {
  try {
    ...
    if (res.responseCode !== 200) {
      ...
      // console.log(
      //   "response in edit post New Modal ",
      //   res.payload.data.post.media
      // );
...
function* getTimeline(params) {
  ...
  try {
    ...
    if (res.responseCode !== 200) {
      ...
      console.log("response in getTimeline success ", res.responseCode);
...
function* fetchComments(params) {
  ...
  try {
    ...
    console.log("saga fetchComments error -_- ", e.toString());
  }
}
...
function* fetchReplies(params) {
  try {
    ...
    if (res.responseCode !== 200) {
      ...
      console.log(
        "response in fetchReplies",
        res.payload.data.replies.data.length
      );
      yield put({
...
function* fetchVideos(params) {
  ...
  try {
    ...
    if (res.responseCode !== 200) {
      ...
      yield put(setVideos(res?.payload?.data?.data));
    }
  } catch (e) {
