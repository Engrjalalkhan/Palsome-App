...
import { editComment } from "../../../../../Redux/actions/NewsFeedActions";
...
const EditItemForDraftComponent = ({
  isVisible,
  onDismiss,
  onPressGetNewData,
  itemId,
  ...
  const [isMarketPlaceCategory, setIsMarketPlaceCategory] = useState("");
  ...
  const setChildCategoriesPickerValue = async (val = "Palsome") => {  
    if (val) {
      if (val !== "Select Country") {
        let conID = childCategories?.find((itm) => itm.name === val);
        let result = conID ? { parent_id: conID.parent_id, id: conID.id } : null;
        
        let id = result ? result.id : null;
  ...
  const onPressPublish = async (val) => {
    setTimeout(() => {
      onDismiss(false);
    }, 500);
    setTimeout(() => {
    ...
    formData.append("country", countryId ? countryId : "");
    formData.append("states", stateId ? stateId : "");
    formData.append("city", cityId ? cityId : "");
    {
      brandInputValue && formData.append("brand", brandInputValue);
    }
    formData.append("parent_category", parentIdEdit);
    formData.append("sub_category", subCategoriesId ? subCategoriesId : "");
    ...
    {
      val === "Draft" && formData.append("add_in_draft", false);
    }
  ...
  const onPressUpdate = async (val) => {
    ...
    var featuresString = featuresArray.join(",");
    {
      ...
      formData.append("add_in_draft", true);
  ...
  const getEditItemApiRequest = async () => {
    ...
    try {
      ...
      if (res?.responseCode === 200) {
        ...
        // setPrice(marketplaceData?.price);
        setPrice(marketplaceData?.price / 100);

        setDescription(marketplaceData?.description);
        setEditCountryId(marketplaceData?.country_id);
        setStateId(marketplaceData?.state_id);
        setCityId(marketplaceData?.city_id);

        setIsMarketPlaceCategory(marketplaceData?.market_place_categories);

        // setSubCategoriesId(marketplaceData?.category_id);
        // setParentId(marketplaceData?.market_place_categories?.parent_id);

        setCountryId(marketplaceData?.country_id);

        setEditStateId(marketplaceData?.state_id);
        setEditCityId(marketplaceData?.city_id);

        setMultipleImages(marketplaceData?.marketmedia);
        setBrandInputValue(marketplaceData?.brand);
        setParentIdEdit(
          res?.payload?.data?.marketplace?.market_place_categories?.parent_id
        );
  ...
  useEffect(() => {
    findTitleById(editCountryId, setCountryValue, country);
    findTitleById(editStateId, setStateValue, state);
    findTitleById(editCityId, setCityValue, city);
  }, [editCountryId, editStateId, editCityId]);

  const findTitleById = (id, setValue, items) => {
    if (items) {
    ...
    >
      ...
      >
        <View style={styles.headerContainer}>
          <Text style={styles.title}>Edit Item For Sale</Text>
          <TouchableOpacity
            onPress={() => {
              onDismiss(false);
            }}
  ...
  >
    <Text style={styles.textStyle}>{t("Publish")}</Text>
  </TouchableOpacity>
  <TouchableOpacity
            // disabled={condition}
...
export default EditItemForDraftComponent;
